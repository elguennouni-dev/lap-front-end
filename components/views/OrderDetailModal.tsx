...
import { Order, Customer, Product, User, UserRole, OrderStatus, TaskStatus, Task, FileAttachment } from '../../types';
...
const OrderDetailModal: React.FC<OrderDetailModalProps> = ({ orderId, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'items' | 'tasks' | 'timeline'>('overview');
  ...
  const getStatusConfig = (status: OrderStatus) => {
    ...
    return configs[status] || { color: 'bg-gray-100 text-gray-700 border-gray-200', icon: 'help', badge: status, gradient: 'from-gray-500 to-gray-600' };
  };
  ...
  const getTaskStatusConfig = (status: TaskStatus) => {
    ...
    return configs[status] || { color: 'bg-gray-100 text-gray-700', icon: 'help' };
  };
        ...
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200 rounded-2xl p-6">
          <div className="flex items-start justify-between">
            <div className="flex items-start space-x-4">
              <div className={`p-3 rounded-xl bg-gradient-to-br ${statusConfig.gradient}`}>
              ...
              <div>
                ...
                <div className="flex items-center space-x-4 mt-3 text-sm text-slate-500">
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <span>Livraison: {new Date(order.delivery_date).toLocaleDateString('fr-FR')}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-slate-200">
            {order.items.map((item, index) => {
              ...
              return (
                <div key={item.order_item_id} className="p-6 hover:bg-slate-50/50 transition-colors">
          ...
          <div className="divide-y divide-slate-200">
            {order.tasks.length > 0 ? (
              order.tasks.map(task => {
                ...
                return (
                  <div key={task.task_id} className="p-6 hover:bg-slate-50/50 transition-colors">
                    ...
                    {hasFiles && (
                      <div className="flex items-center space-x-2 text-blue-600 text-sm bg-blue-50 rounded-lg px-3 py-2">
                        <Icon name="attachment" className="h-4 w-4" />
                        <span>{task.design_files!.length} fichier(s) design attaché(s)</span>
                      </div>
                    )}
                  </div>
                );
              ...
              <div className="p-8 text-center">
                <Icon name="assignment-late" className="h-12 w-12 text-slate-300 mx-auto mb-3" />
  ...
  const renderActions = () => {
    ...
    return (
      <div className="bg-white border-t border-slate-200 p-6 space-y-4">
        ...
        {isAdmin && (
          <>
            {order.status === OrderStatus.NEW_ORDER && (
              <div className="space-y-3">
                ...
                <button 
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-3 px-6 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center space-x-2 shadow-lg shadow-blue-500/25"
                >
                  <Icon name="assignment" className="h-4 w-4" />
                  <span>{actionLoading ? 'Assignation...' : 'Assigner Design'}</span>
                </button>
              </div>
            )}
            ...
            {order.status === OrderStatus.DESIGN_PENDING_APPROVAL && (
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  <Icon name="approval" className="h-5 w-5 text-slate-600" />
                  <label className="text-sm font-medium text-slate-800">Approbation Design</label>
                </div>
            ...
            {order.status === OrderStatus.DESIGN_APPROVED && (
              <div className="space-y-3">
                ...
                >
                  ...
                  <span>{actionLoading ? 'Assignation...' : 'Assigner Production'}</span>
                </button>
              </div>
            )}
            ...
            {order.status === OrderStatus.PRODUCTION_COMPLETE && (
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  <Icon name="next" className="h-5 w-5 text-slate-600" />
        ...
        {userTasks.map(task => (
          <div key={task.task_id} className="border border-yellow-200 bg-yellow-50 rounded-xl p-4 space-y-3">
            <div className="flex items-center space-x-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                ...
                className="bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200"
              >
                Accepter
              </button>
              <button 
                onClick={() => handleAction(() => api.updateTaskStatus(order.order_id, task.task_id, TaskStatus.REJECTED))}
                className="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200"
              >
                Rejeter
              </button>
            </div>
         ...
         (order.status === OrderStatus.PRODUCTION_IN_PROGRESS && order.tasks.some(t => t.assigned_to === currentUser.user_id && t.step_name === 'Production' && t.status === TaskStatus.ACCEPTED)) ? (
          <div className="space-y-3">
            <div className="flex items-center space-x-2">
              ...
              <label className="text-sm font-medium text-slate-800">Progression de la tâche</label>
            </div>
            <button 
              onClick={() => handleAction(() => api.updateTaskStatus(order.order_id, order.tasks.find(t => t.assigned_to === currentUser.user_id && t.status === TaskStatus.ACCEPTED)!.task_id, TaskStatus.COMPLETED))}
              className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center space-x-2 shadow-lg shadow-blue-500/25"
            >
              <Icon name="check-circle" className="h-4 w-4" />
    ...
    return (
      <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50" onClick={() => onClose()}>
        <div className="bg-white rounded-2xl p-8 max-w-md text-center" onClick={e => e.stopPropagation()}>
          <Icon name="error" className="h-16 w-16 text-red-500 mx-auto mb-4" />
          <h3 className="text-xl font-bold text-slate-800 mb-2">Commande non trouvée</h3>
          <p className="text-slate-600 mb-4">La commande que vous recherchez n'existe pas ou a été supprimée.</p>
          <button 
            onClick={() => onClose()}
            className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-2 px-6 rounded-xl transition-all duration-200"
          >
            Fermer
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4" onClick={() => onClose(order)}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="p-8 border-b border-slate-200">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-3xl font-bold text-slate-800">Détails de la Commande</h2>
              <p className="text-slate-600 text-lg mt-2">Gestion et suivi de la commande</p>
            </div>
            <button 
        ...
        <div className="border-b border-slate-200">
          <div className="px-8 flex space-x-8">
            {[
              ...
              { id: 'items', label: 'Articles', icon: 'shopping-cart' },
              { id: 'tasks', label: 'Tâches', icon: 'assignment' },
            ].map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as any)}
                className={`flex items-center space-x-3 py-5 border-b-2 transition-colors text-lg ${
                  activeTab === tab.id
